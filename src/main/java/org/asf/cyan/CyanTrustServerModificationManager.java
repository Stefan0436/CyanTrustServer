...
public abstract class CyanTrustServerModificationManager extends CyanComponent {

	private static CyanTrustServerModificationManager instance;
	private static boolean prepared = false;

	/**
	 * Assigns the instance.
	 */
	protected static void assign(CyanTrustServerModificationManager instance) {
		CyanTrustServerModificationManager.instance = instance;
	}
				...
				try {
					if (cmd.equals("processor")) {
						...
						instance.registerVirtualFile(
								(IVirtualFileProvider) Class.forName(arguments.get(0).substring("class:".length()),
										false, instance.getClass().getClassLoader()).getConstructor().newInstance());
